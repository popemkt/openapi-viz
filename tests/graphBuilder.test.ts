...
describe('buildGraph - Relationship-Based Mode', () => {
  describe('Schema-to-Schema edges', () => {
    it('creates property edges with semantic labels', () => {
      const relationships = [
        createRelationship(
          ...
          schemaRef('User'),
          schemaRef('Address'),
          { propertyName: 'homeAddress', required: true }
        ),
      ];
    ...
    it('creates oneOf composition edges (dashed)', () => {
      ...
      const spec = createParsedSpec(
        [],
        [createSchema('Pet'), createSchema('Dog')],
        relationships
      );
      ...
      const edge = edges.find(
        (e) => e.source === 'schema-Pet' && e.target === 'schema-Dog'
      );
      expect(edge).toBeDefined();
    ...
    it('creates anyOf composition edges (dashed)', () => {
      ...
      const edge = edges.find(
        (e) => e.source === 'schema-Response' && e.target === 'schema-Error'
      );
      expect(edge).toBeDefined();
    ...
    it('creates discriminator edges with value context', () => {
      ...
      const dogEdge = edges.find(
        (e) => e.source === 'schema-Pet' && e.target === 'schema-Dog' && e.data?.edgeType === 'discriminator'
      );
      const catEdge = edges.find(
        (e) => e.source === 'schema-Pet' && e.target === 'schema-Cat' && e.data?.edgeType === 'discriminator'
      );
    ...
    it('creates not schema edges', () => {
      ...
      const edge = edges.find(
        (e) => e.source === 'schema-NotEmpty' && e.target === 'schema-Empty'
      );
      expect(edge).toBeDefined();
    ...
    it('creates response edges with status code', () => {
      const relationships = [
        ...
        createRelationship(
          ...
          endpointRef('get-/users'),
          schemaRef('Error'),
          { statusCode: '400', mediaType: 'application/json' }
        ),
      ];
      ...
      const okEdge = edges.find(
        (e) => e.source === 'get-/users' && e.target === 'schema-UserList'
      );
      ...
      const errorEdge = edges.find(
        (e) => e.source === 'get-/users' && e.target === 'schema-Error'
      );
  ...
  describe('Node data enrichment', () => {
    ...
    it('sets compositionType from schema', () => {
      ...
      const spec = createParsedSpec(
        [],
        [allOfSchema, oneOfSchema, anyOfSchema],
        []
      );
  ...
  describe('Edge deduplication', () => {
    ...
    it('keeps relationships with different contexts', () => {
      ...
      expect(userToAddressEdges.some((e) => e.data?.semanticContext?.propertyName === 'homeAddress')).toBe(true);
      expect(userToAddressEdges.some((e) => e.data?.semanticContext?.propertyName === 'workAddress')).toBe(true);
    });
  });
